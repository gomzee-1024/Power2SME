 ...
 */
public class CustomerSearchResultsFragment extends SuperFragment implements ICustomerSelectionView
{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
    {
        ...
        customersList = getFragmentDataBundle().getParcelableArrayList(Constants.BUNDLE_KEY_SEARCHED_CUSTOMERS);
        sustomerSearchFilter = CustomerSearchFilter.getFilter(getFragmentDataBundle().getString(Constants.BUNDLE_KEY_KAM_FILTER));
        searchQuery = getFragmentDataBundle().getString(Constants.BUNDLE_KEY_KAM_CUSTOMER_QUIRY);
        return rootView;
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState)
    {
        ...
        {
            ResponseCustomersDto dto = new ResponseCustomersDto();
            dto.setTotalRecord(""+customersList.size());
            dto.setCustomers(customersList);
            showCustomers(dto, false);
        }
    }
...
//        menu3.hideMenuButton(false);
        ...
        ListView_customersList.setRefreshingColorResources(android.R.color.holo_orange_light, android.R.color.holo_blue_light, android.R.color.holo_green_light, android.R.color.holo_red_light);
        ListView_customersList.setupMoreListener(new OnMoreListener() {
            @Override
    ...
    public void showUIMessage(UIMessage uiMessage, int flag)
    {
        ...
        if(!showErrorMessageFragment(uiMessage.getUiMessageType(), CustomerSearchResultsFragment.class, null))
        {
...
//            UIMessageUtility.displayUIMessage(baseActivity, uiMessage);
    ...
    public void showCustomers(ResponseCustomersDto responseCustomersDto, boolean isLoadmore)
    {
        ...
        ListView_customersList.getSwipeToRefresh().setEnabled(true);
        TextView_transactedCustomersLabel.setText(getString(R.string.kam_search_label_search_results));
        ...
        if(responseCustomersDto!=null)
        {
            ...
            if(customerList!=null)
            {
                ...
                TextView_displayedResults.setText("" + adapter.getItemCount()+" of "+customersTotalRecordsCount+" results");
            }
        }
        else {
            adapter.clear();
            adapter.notifyDataSetChanged();
            TextView_displayedResults.setText("0 of 0 results");
        }
    }
